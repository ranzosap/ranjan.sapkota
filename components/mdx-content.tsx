...
export function MdxContent({ children, content, code }: MdxContentProps) {
  ...
  const renderMarkdown = (src: string) => {
    ...
    html = html.replace(/!\[(.*?)\]\((\S+?)(?:\s+"(.*?)")?\)/g, (_m, alt: string, src: string, title: string | undefined) => {
      ...
      if (isVideo) {
        return `<div style="position:relative;width:100%;max-width:100%;aspect-ratio:16/9;"><video src="${src}" controls preload="none" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;"></video></div>`
      }
      if (isAudio) {
